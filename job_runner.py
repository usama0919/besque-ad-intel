...
RUN_INSTRUCTION, RUN_CHECK_OUTPUT, RUN_RETHEME_COLOURS from the environment.
"""
import os
...
def main():
    ...
    check_output = os.getenv("RUN_CHECK_OUTPUT") == "1"
    # Absent env var must still mean True (the default) - matching include_product's
    # pattern above - only an explicit "0" turns re-theming off.
    retheme_colours = os.getenv("RUN_RETHEME_COLOURS", "1") != "0"
    print(f">> Job starting: competitor_id={competitor_id}, max_per_competitor={n}, "
          ...
          f"operator_instruction={operator_instruction!r}, check_output={check_output}, "
          f"retheme_colours={retheme_colours}")
