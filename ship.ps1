...
# --no-allow-unauthenticated is left exactly as-is - that decision is separate from
# this secret-wiring change. --set-secrets replaces the old plain
# --update-env-vars SHARED_ACCESS_PASSWORD=... (never used - this is the first time
# this var reaches deploy) with a Secret Manager reference, so the value is never
# visible in `gcloud run services describe` output or deploy logs the way a plain
# env var would be. --max-instances lowered 5 -> 3 (see the pool-size math recorded
# alongside this change) - --min-instances 1 is unchanged, kept explicit here rather
# than left to a flag default.
gcloud run deploy besque-dashboard --source . --region $region --project $project --no-allow-unauthenticated --add-cloudsql-instances besque-martech:europe-west2:besque-db --update-env-vars STORAGE_BACKEND=gcs --update-env-vars GCS_BUCKET=besque-ad-intel-assets --set-secrets "SHARED_ACCESS_PASSWORD=${secretName}:latest" --no-cpu-throttling --min-instances 1 --max-instances 3
