...
STYLE_GUIDANCE = {
    ...
    "ugc": (
        ...
        "over-smooth it. In-image text (STRICT, 2026-08-13, Item 3): any caption or quote "
        "rendered directly onto this image reads as someone typing on their own phone, "
        "never as marketing copy laid over a photo. No formal quotation marks around a "
        "first-person statement - a real caption doesn't wrap itself in quotes. No "
        "em-dash attribution line ('— Name') - a genuine phone caption states a name "
        "plainly if at all, never set off like a pull-quote. Capitalisation is casual and "
        "inconsistent, never a clean capitalised sentence - a lowercase start or a missed "
        "capital on a proper noun is authentic here, not a mistake to correct. Punctuation "
        "is plain - periods and commas, never a stylised ellipsis used as decorative "
        "pause. Never a rhetorical marketing question ('Sound familiar?', 'Ready to "
        "glow?') - a real person describing their own experience doesn't ask the viewer a "
        "leading question. This governs how supplied text renders, never what text is "
        "sourced or says - a real testimonial is still required and its actual words are "
        "never altered, only how it is typeset. Post-processing (STRICT, 2026-08-12): NO retouching, NO AI "
    ...
    "high_spec": (
        "Framing: high-end studio product photograph, professional premium-brand product "
        "photography, polished, ultra-realistic, ultra-high production value. Lighting: soft "
        "diffused key light with subtle rim lighting, sharp studio lighting with a clean "
        "reflection on the glass, gentle highlights, shallow depth of field with the subject in "
        "sharp focus. 'Dramatic' and 'cinematic' both belong here, not in ugc. Composition: "
        "clean and minimal, sophisticated and luxurious, a high-end premium-brand studio "
        "aesthetic - polished production value only, never a model-age or fashion-editorial "
        "styling cue (rule 10 elsewhere in this prompt governs the subject's own required "
        "appearance regardless of how polished this register otherwise looks; this register "
        "is a lighting/production-quality descriptor, not a casting direction). Describe only "
        "how the product is lit and shot, never its own geometry or label (fixed elsewhere): "
        "realistic glass reflections, accurate light falloff, natural shadows, sharp label "
        "detail, true-to-life material texture."
    ),
    "illustrated": (
        ...
        "fidelity: the BOTTLE IDENTITY rule stated earlier in this prompt is what the label "
        "actually CONTAINS - this register note governs only what renders LEGIBLY at this "
        "scale/style, never different content. Illustrated styles tend to abstract the "
        "product's label into unreadable scribbles unless told otherwise - explicitly state "
        "that the bottle stays RECOGNISABLE by silhouette, colour, and the product's NAME on "
        "the label (that much stays legible), rendered in THIS SCENE'S OWN illustrated visual "
        "language, never photorealistic and never a photographic bottle composited into the "
        "drawing - a 3D/Pixar scene gets a 3D/Pixar-rendered label, a vintage comic scene gets "
        "the same bold-outline/halftone treatment, a flat vector scene gets the same "
        "flat-shaded treatment. Secondary label content ONLY - sub-lines, certification "
        "icons, fine print - does NOT need to stay legible at this scale in this style; "
        "demanding it produced exactly the unreadable-scribble failure this note exists to "
        "prevent (2026-08-06, a real Grüns GLP-1 illustrated draft composited a "
        "photorealistic bottle into a hand-drawn scene specifically to keep that secondary "
        "text legible). Name and colour accuracy matter; secondary-text legibility does not, "
        "in this register only - and this is a legibility exception only, never a licence to "
        "render a different bottle, wordmark, or colour scheme. Never describe what the "
        "label actually says or looks like here - that comes from the BOTTLE IDENTITY rule "
        "above and the bottle-fixed clause immediately following this note."
    ),
}
