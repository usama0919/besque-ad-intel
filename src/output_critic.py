...
CRITIC_SYSTEM = (
    ...
    "- BORROWED PERSONAL ATTRIBUTION (C9): a personal name, initial-surname construction "
    "(e.g. \"Sean R.\"), handle, or signature rendered anywhere in the GENERATED image "
    "that is NOT the one authorised testimonial's own attribution supplied below (if "
    "one is supplied). This is a DIFFERENT category from the testimonial check above: "
    "that one covers a FABRICATED quote's content; this one covers a REAL person's name "
    "carried over from the reference ad even when the surrounding brand and product were "
    "correctly substituted - confirmed live, a draft correctly substituted the "
    "competitor's brand/product throughout an ad but the competitor's own testimonial "
    "attribution survived verbatim. Flag this even on an otherwise clean draft, and even "
    "when no testimonial-shaped quote accompanies the name - a bare name with no quote "
    "around it is still a violation of this category\n"
