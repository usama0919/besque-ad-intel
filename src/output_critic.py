...
CRITIC_SYSTEM = (
    ...
    "- BORROWED PERSONAL ATTRIBUTION OR ACCOUNT IDENTITY (C9): a personal name, "
    "initial-surname construction (e.g. \"Sean R.\"), signature, social media @handle or "
    "username (e.g. \"@fitness_ty\"), or profile/display name rendered anywhere in the "
    "GENERATED image that is NOT the one authorised testimonial's own attribution "
    "supplied below (if one is supplied). This is a DIFFERENT category from the "
    "testimonial check above: that one covers a FABRICATED quote's content; this one "
    "covers a REAL person's or REAL account's identity carried over from the reference "
    "ad even when the surrounding brand and product were correctly substituted - "
    "confirmed live twice: a draft correctly substituted the competitor's brand/product "
    "throughout an ad but the competitor's own testimonial attribution survived "
    "verbatim, and a SEPARATE draft carried the competitor's real Instagram handle "
    "verbatim. Flag this even on an otherwise clean draft, and even when no "
    "testimonial-shaped quote accompanies the name/handle - a bare name or handle with "
    "no quote around it is still a violation of this category. Where the reference "
    "shows account chrome (an avatar, handle, verified tick, follow button - a "
    ...
    "notice only incidentally while checking something else. \"Any human subject\" "
    "INCLUDES a face rendered inside a small avatar/profile picture within account "
    "chrome (a UGC-style testimonial/social-post card), not only the ad's primary, "
    "large-in-frame subject - an avatar is a depicted person, not decorative UI "
    "furniture exempt from this rule; confirmed live, an identifiable-looking avatar "
    "reading mid-30s shipped unflagged on a draft this exact check was supposed to "
    "cover. Look specifically for: "
    ...
    "face, same hair, same clothing, same pose. Applies to ANY face in the draft, "
    "INCLUDING a small avatar/profile-picture face within reproduced account chrome, "
    "not only the ad's primary, large-in-frame subject - an avatar face copied from the "
    "reference's own account chrome is exactly the same identity violation as copying "
    "the primary subject's face, just at a smaller scale. The reference's model (and "
    "any avatar's subject) must be SUBSTITUTED, never reproduced (compliance rule C1, "
    "rule 10 above) - this is a rights/identity violation, not a fidelity choice. Judge "
    "holistically: even if no single feature is an exact pixel match, a strong overall "
    "impression of the same individual (same facial structure, same hair colour/style, "
    "same wardrobe, same body position) is enough to flag - do not require every "
    "feature to match before reporting this. This category ONLY applies when a "
    "reference image is actually attached alongside the draft (edit mode) - there is "
    "nothing to compare against otherwise, and this category should not fire without a "
    ...
    "the surrounding scene. This has THREE distinct shapes, all count: (1) a photorealistic "
    ...
    "the scene and the product are photographic; (3) the oil/liquid or the bottle's own "
    "materials read as flat and synthetic rather than physically real - the oil showing no "
    "visible liquid volume or surface line (meniscus) where it meets the air, no "
    "translucency or viscosity (a flat, uniform, matte fill instead of real liquid), the "
    "glass showing no refraction or reflection of the surrounding scene's light (reading "
    "as a flat sticker rather than a transparent object), pump/cap hardware with no real "
    "specular highlight or material response, or a label that reads as a flat decal pasted "
    "onto the bottle rather than a real label wrapped around its curve, warped or "
    "distorted, or illegible from the wrap itself. The bottle's geometry and label CONTENT "
    "must stay accurate in every case, but its RENDERING - lighting, shadow, grain, depth "
